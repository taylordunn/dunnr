...
The `set_palette()` function is a convenience way to quickly set the default discrete palette (via `ggplot2.discrete.fill` and `ggplot2.continuous.fill` options) and the default continuous palette (via `ggplot2.continuous.fill` and `ggplot2.continuous.colour`):
